...
class AppointmentApi {
  ...
  Future<ResponseWrapper<List<Appointment>>> retrieveUpcomingAppointment({required String patientId}) async {
    String url = TemplateString(stringWithParams: ApiEndPoint.APPOINTMENT_UPCOMING_LIST, params: {'patientId': patientId}).toString();
  ...
  Future<ResponseWrapper<List<Appointment>>> retrievePastAppointment({required String patientId}) async {
    String url = TemplateString(stringWithParams: ApiEndPoint.APPOINTMENT_PAST_LIST, params: {'patientId': patientId}).toString();
  ...
  Future<ResponseWrapper<Appointment>> updateAppointment({required String patientId, required String appointmentId, required String status}) async {
    String url = TemplateString(stringWithParams: ApiEndPoint.APPOINTMENT_CANCEL, params: {'patientId': patientId, 'appointmentId': appointmentId}).toString();
    var data = {
      'status': status,
    };
    var options = await ApiUtil.generateDioOptions();
