...
///  * [TabBar], which displays a row of tabs.
///  * [CustomTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [TabBar] and a [CustomTabBarView].
...
class _CustomIndicatorPainter extends CustomPainter {
  _CustomIndicatorPainter({
    ...
    required _CustomIndicatorPainter? old,
  ...
  @override
  bool shouldRepaint(_CustomIndicatorPainter old) {
    return _needsPaint
...
class _CustomChangeAnimation extends Animation<double> with AnimationWithParentMixin<double> {
  _CustomChangeAnimation(this.controller);
...
class _CustomDragAnimation extends Animation<double> with AnimationWithParentMixin<double> {
  _CustomDragAnimation(this.controller, this.index);
...
/// This sample shows the implementation of [TabBar] and [CustomTabBarView] using a [DefaultTabController].
...
/// to control the behavior of the [TabBar] and [CustomTabBarView]. This can be used instead of
...
///  * [TabBar], which is used to display the [Tab] that corresponds to each page of the [CustomTabBarView].
...
class _CustomTabBarState extends State<CustomTabBar> {
  ...
  void _initIndicatorPainter() {
    _indicatorPainter = !_controllerIsValid ? null : _CustomIndicatorPainter(
      controller: _controller!,
      indicator: _indicator,
      indicatorSize: widget.indicatorSize ?? TabBarTheme.of(context).indicatorSize,
      // indicatorSize: widget.indicatorSize ?? TabBarTheme.of(context).indicatorSize as TabBarIndicatorSize,
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller != null) {
      ...
      if (_controller!.indexIsChanging) {
        ...
        final Animation<double> animation = _CustomChangeAnimation(_controller!);
        ...
        final Animation<double> centerAnimation = _CustomDragAnimation(_controller!, tabIndex);
        wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex], true, centerAnimation);
        if (_currentIndex! > 0) {
          final int tabIndex = _currentIndex! - 1;
          final Animation<double> previousAnimation = ReverseAnimation(_CustomDragAnimation(_controller!, tabIndex));
          wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex], false, previousAnimation);
        }
        if (_currentIndex! < widget.tabs.length - 1) {
          final int tabIndex = _currentIndex! + 1;
          final Animation<double> nextAnimation = ReverseAnimation(_CustomDragAnimation(_controller!, tabIndex));
